...
function useTweaksDR() {
  const [t, setT] = useStateDR(() => {
    try { const saved = JSON.parse(localStorage.getItem('dr-tweaks-v2') || '{}'); return { ...DEFAULTS, ...saved }; }
    catch { return DEFAULTS; }
  });
  useEffectDR(() => { try { localStorage.setItem('dr-tweaks-v2', JSON.stringify(t)); } catch {} }, [t]);
...
function TweaksPanelDR({ t, set }) {
  ...
  if (!open) return (
    <button onClick={()=>setOpen(true)} style={{ position:'fixed',right:20,bottom:20,height:38,padding:'0 14px',borderRadius:10,border:'1px solid #e4e4e7',background:'#fff',fontFamily:'Geist',fontSize:13,color:'#0c0c1d',cursor:'pointer',boxShadow:'0 6px 16px -4px rgba(0,0,0,0.1)',zIndex:9999 }}>⚙ Tweaks</button>
  );
  return (
    <div className="DR-tweaks">
      ...
      <div className="DR-row">
        ...
        <div className="DR-seg">
          {[['light','Light'],['dark','Dark']].map(([v,l])=>(
            <button key={v} className={t.theme===v?'on':''} onClick={()=>set({theme:v})}>{l}</button>
          ))}
        </div>
      </div>
      <div className="DR-row">
        ...
        <div className="DR-seg">
          {[['compact','Compact'],['regular','Regular'],['comfortable','Comfort']].map(([v,l])=>(
            <button key={v} className={t.density===v?'on':''} onClick={()=>set({density:v})}>{l}</button>
          ))}
        </div>
      </div>
      <div className="DR-row">
        <label>Sidebar</label>
        <div className="DR-seg">
          {[['full','Full'],['mini','Mini']].map(([v,l])=>(
            <button key={v} className={t.sidebar===v?'on':''} onClick={()=>set({sidebar:v})}>{l}</button>
          ))}
        </div>
      </div>
      <div className="DR-row" style={{ marginBottom: 0 }}>
        <label>Accent</label>
        <div className="DR-sw">
          {[['indigo','#6366f1'],['violet','#8b5cf6'],['cyan','#0ea5e9']].map(([v,c])=>(
            <button key={v} className={t.accent===v?'on':''} onClick={()=>set({accent:v})}>
              <span className="dot" style={{ background: c }}/>{v}
            </button>
          ))}
        </div>
      </div>
...
function AppDR() {
  ...
  useEffectDR(() => {
    const handler = (e) => {
      if (e.data?.type === '__activate_edit_mode') {/* panel visible by default */}
    };
  ...
  const screens = [
    { id: 'auth',         label: '00 · Login / Cadastro',     w: 1240, h: 780, render: () => <AuthA tweaks={t}/> },
    { id: 'dashboard',    label: '01 · Dashboard',             w: 1320, h: 980, render: () => <DashboardA tweaks={t}/> },
    { id: 'transactions', label: '02 · Transações',            w: 1320, h: 980, render: () => <TransactionsA tweaks={t}/> },
    { id: 'categories',   label: '03 · Categorias',            w: 1320, h: 780, render: () => <CategoriesA tweaks={t}/> },
    { id: 'savings',      label: '04 · Caixinhas / Metas',     w: 1320, h: 820, render: () => <SavingsA tweaks={t}/> },
    { id: 'profile',      label: '05 · Perfil',                w: 1320, h: 880, render: () => <ProfileA tweaks={t}/> },
  ];

  return (
    <>
      <DesignCanvas title="Expense Tracker · Direção A (Clean)" subtitle="Redesign completo — sidebar, dashboard, transações, categorias, caixinhas, perfil e auth">
        <DCSection id="auth" title="Entrada · Login & Cadastro">
          <DCArtboard id="auth" label={screens[0].label} width={screens[0].w} height={screens[0].h}>{screens[0].render()}</DCArtboard>
        </DCSection>
        <DCSection id="core" title="App · Telas principais">
          {screens.slice(1, 5).map(s => (
            <DCArtboard key={s.id} id={s.id} label={s.label} width={s.w} height={s.h}>{s.render()}</DCArtboard>
          ))}
        </DCSection>
        <DCSection id="account" title="Conta">
          <DCArtboard id="profile" label={screens[5].label} width={screens[5].w} height={screens[5].h}>{screens[5].render()}</DCArtboard>
        </DCSection>
      </DesignCanvas>
