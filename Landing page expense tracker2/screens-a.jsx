...
const CATEGORIES = [
  ...
  { name: 'Pix',          pct: 30, color: '#8b5cf6', emoji: '⚡', total: 388.10, count: 8 },
...
const MONTHLY = [
  { m:'Jan', inc:3100, exp:1420 },{ m:'Fev', inc:3250, exp:1890 },{ m:'Mar', inc:2980, exp:2100 },
  { m:'Abr', inc:3179, exp:1293 },{ m:'Mai', inc:3400, exp:1650 },{ m:'Jun', inc:3380, exp:1480 },
  { m:'Jul', inc:3200, exp:1720 },{ m:'Ago', inc:3500, exp:1580 },{ m:'Set', inc:3420, exp:1390 },
  { m:'Out', inc:3180, exp:1250 },{ m:'Nov', inc:3600, exp:1700 },{ m:'Dez', inc:3800, exp:2200 },
];
const TXS = [
  { desc:'Compra no débito via NuPay — iFood', cat:'Alimentação',  date:'22/04/2026', value:-33.39 },
  { desc:'Compra no débito — CIA DAS TORTAS',  cat:'Débito',        date:'22/04/2026', value:-9.60 },
  { desc:'Transferência Pix — ZAMP S.A.',      cat:'Pix',           date:'21/04/2026', value:25.90 },
  { desc:'Compra no débito — Uber *TRIP',      cat:'Transporte',    date:'21/04/2026', value:-17.36 },
  { desc:'Supermercado Extra — Cartão',        cat:'Supermercado',  date:'20/04/2026', value:-142.80 },
  { desc:'Farmácia Drogasil',                  cat:'Saúde',         date:'18/04/2026', value:-48.50 },
  { desc:'Cinemark — Shopping Tamboré',        cat:'Lazer',         date:'15/04/2026', value:-62.00 },
  { desc:'Salário — CLT',                      cat:'Receita',       date:'05/04/2026', value:3179.00 },
];
const CAT_EMOJI = { 'Alimentação':'🍔','Transporte':'🚗','Supermercado':'🛒','Pix':'⚡','Receita':'💰','Débito':'💳','Saúde':'💊','Lazer':'🎬','Educação':'📚' };
...
function Donut({ segs, centerLabel, centerValue, size=140, stroke=18 }) {
  const R=(size-stroke)/2, C=2*Math.PI*R; let acc=0;
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <circle cx={size/2} cy={size/2} r={R} fill="none" stroke="var(--a-divider)" strokeWidth={stroke}/>
      {segs.map((s,i)=>{const len=(s.pct/100)*C; const el=(
      ...
      <text x={size/2} y={size/2-4} textAnchor="middle" fontSize="10" fill="var(--a-ink-400)" fontFamily="Geist Mono" letterSpacing="1">{centerLabel}</text>
      <text x={size/2} y={size/2+16} textAnchor="middle" fontSize="18" fontWeight="600" fill="var(--a-ink-900)" fontFamily="Geist" letterSpacing="-0.02em">{centerValue}</text>
    </svg>
  );
}
      ...
      <main className="A-main">
        <div className="A-top">
          <div>
            ...
            <div className="sub">Você gastou <b style={{ color: 'var(--a-ink-900)' }}>R$ 1.292,83</b> em abril · 41% da sua receita</div>
          </div>
        ...
        <div className="A-kpis">
          {KPIS.map((k,i) => {
            const spark=[32,41,28,52,45,63,58,72]; const mx=Math.max(...spark);
            ...
            return (
              <div key={k.label} className="A-kpi">
                ...
                <div className={'dlt '+(k.delta.startsWith('-')?'neg':'pos')}>{k.delta} vs. mês anterior</div>
                <svg className="spark" viewBox="0 0 100 24" preserveAspectRatio="none">
                  <polyline fill="none" stroke={color} strokeWidth="1.5"
                    points={spark.map((v,j)=>`${(j/(spark.length-1))*100},${24-(v/mx)*22}`).join(' ')}/>
                </svg>
              </div>
            );
          })}
        ...
        <div className="A-row">
          <div className="A-card">
            <div className="A-card-h"><div><h3>Evolução Mensal</h3><div style={{ fontSize:12,color:'var(--a-ink-500)',marginTop:2 }}>Receitas vs. Despesas · 12 meses</div></div><div className="meta">Jan → Dez</div></div>
            ...
            <div style={{ display:'flex',gap:18,marginTop:14,fontSize:12,color:'var(--a-ink-500)' }}>
              <span><span style={{ display:'inline-block',width:8,height:8,background:'var(--a-ink-900)',borderRadius:2,marginRight:6 }}/>Receitas</span>
              <span><span style={{ display:'inline-block',width:8,height:8,background:'var(--a-divider)',borderRadius:2,marginRight:6 }}/>Despesas</span>
            </div>
          </div>

          <div className="A-card">
            <div className="A-card-h"><h3>Distribuição</h3><div className="meta">R$ 1.293</div></div>
            <div style={{ display:'flex',alignItems:'center',gap:16 }}>
              ...
              <div style={{ flex:1,display:'flex',flexDirection:'column',gap:8,fontSize:12.5 }}>
                {CATEGORIES.slice(0,5).map(c=>(
                  <div key={c.name} style={{ display:'flex',justifyContent:'space-between',color:'var(--a-ink-700)' }}>
                    <span><span style={{ display:'inline-block',width:8,height:8,borderRadius:2,background:c.color,marginRight:8 }}/>{c.name}</span>
                    <span style={{ fontFamily:'Geist Mono, monospace',color:'var(--a-ink-500)' }}>{c.pct}%</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="A-card A-insight">
            <div className="head"><span className="i">✱</span>Insights</div>
            <div className="body">Você gastou <em>R$ 187</em> a menos em <em>Alimentação</em> este mês vs. o trimestre. Continue assim para atingir sua meta de <em>R$ 1.200</em>.</div>
            <div style={{ marginTop:4 }}>
              <div style={{ display:'flex',justifyContent:'space-between',fontSize:11.5,color:'var(--a-ink-500)',fontFamily:'Geist Mono, monospace' }}><span>META MENSAL</span><span>78%</span></div>
              <div className="goal-bar"><div className="goal-fill" style={{ width:'78%' }}/></div>
              <div style={{ fontSize:11.5,color:'var(--a-ink-400)',marginTop:6,fontFamily:'Geist Mono, monospace' }}>R$ 936 de R$ 1.200</div>
            </div>
            <div className="head" style={{ marginTop:10 }}><span className="i" style={{ background:'rgba(16,185,129,0.15)',color:'#10b981' }}>↗</span>Previsão</div>
            <div className="body" style={{ fontSize:13.5 }}>No ritmo atual você fecha Abril com <em>R$ 1.450</em> em despesas.</div>
          </div>
        </div>
        ...
        <div className="A-card">
          <div className="A-card-h">
            ...
            <div style={{ display:'flex',gap:8,alignItems:'center' }}>
              <div className="A-search" style={{ width:220 }}><span style={{ color:'var(--a-ink-400)',fontSize:12 }}>⌕</span><input placeholder="Buscar por descrição..."/></div>
          ...
          {TXS.slice(0,6).map((t,i)=>(
            <div key={i} className="A-txrow">
              ...
              <div><div className="desc">{t.desc}</div><div className="meta">{t.date}</div></div>
              <div className="cat">{t.cat}</div>
              <div className={'val '+(t.value>0?'pos':'')}>{t.value>0?'+':'−'} {fmt(t.value)}</div>
            </div>
          ))}
        </div>
      </main>
...
function TransactionsA({ tweaks }) {
  ...
  return (
    <Shell active="transactions" tweaks={tweaks}>
      <style>{DASH_STYLE}</style><style>{TX_STYLE}</style>
      <main className="A-main">
        <div className="A-top">
          <div><h1>Transações</h1><div className="sub">{all.length} registros · Abril 2026</div></div>
          <div style={{ display:'flex',gap:6 }}>
        ...
        <div className="A-tx-stats">
          <div className="A-tx-stat"><div className="l">Total no período</div><div className="v" style={{ color:'#ef4444' }}>− R$ 1.292,83</div></div>
          <div className="A-tx-stat"><div className="l">Receitas</div><div className="v" style={{ color:'#10b981' }}>+ R$ 3.179,00</div></div>
          <div className="A-tx-stat"><div className="l">Maior gasto</div><div className="v">R$ 142,80</div></div>
          <div className="A-tx-stat"><div className="l">Média diária</div><div className="v">R$ 43,09</div></div>
        </div>
        ...
        <div className="A-tx-table">
          <div className="A-tx-head">
            <span/><span>Descrição</span><span>Categoria</span><span>Data</span><span style={{ textAlign:'right' }}>Valor</span><span/>
          </div>
          <div className="A-tx-body">
            {all.map((t,i)=>(
              <div key={i} className="A-txrow">
                <div className="ic">{CAT_EMOJI[t.cat]||'•'}</div>
                <div><div className="desc">{t.desc}</div><div className="meta">Conta Corrente · 2372</div></div>
                <span className="A-tx-tag" style={{ background: 'color-mix(in oklab, var(--accent) 10%, var(--a-hover))', color: 'var(--accent)' }}>{t.cat}</span>
                <div className="cat" style={{ textTransform:'none' }}>{t.date}</div>
                <div className={'val '+(t.value>0?'pos':'')}>{t.value>0?'+':'−'} {fmt(t.value)}</div>
...
function CategoriesA({ tweaks }) {
  return (
    <Shell active="categories" tweaks={tweaks}>
      ...
      <main className="A-main">
        <div className="A-top">
          <div><h1>Categorias</h1><div className="sub">{CATEGORIES.length} categorias · {CATEGORIES.reduce((a,c)=>a+c.count,0)} transações classificadas</div></div>
          <div style={{ display:'flex',gap:6 }}>
            <button className="A-chip">📅 Abril 2026</button>
          ...
          {CATEGORIES.map(c=>(
            <div key={c.name} className="A-cat-card">
              <div className="A-cat-top">
                <div className="A-cat-emoji" style={{ background: `color-mix(in oklab, ${c.color} 15%, transparent)` }}>{c.emoji}</div>
              ...
              <div>
                ...
                <div style={{ display:'flex',justifyContent:'space-between',fontSize:11,color:'var(--a-ink-500)',fontFamily:'Geist Mono, monospace',marginTop:8,marginBottom:6 }}><span>{c.pct}% DO TOTAL</span></div>
                <div className="A-cat-bar"><span style={{ width: `${c.pct*2.5}%`, background: c.color }}/></div>
              </div>
            </div>
          ))}
...
function SavingsA({ tweaks }) {
  const cx = [
    ...
    { emoji:'🛡️', name:'Reserva de emergência', color:'#10b981', cur:9800, tgt:15000, eta:'6 meses' },
  ];
  return (
    <Shell active="savings" tweaks={tweaks}>
      ...
      <main className="A-main">
        <div className="A-top">
          <div><h1>Caixinhas</h1><div className="sub">4 objetivos · R$ 40.300 reservados · 42% da meta total</div></div>
          <div style={{ display:'flex',gap:6 }}>
        ...
        <div className="A-sv-top-stats">
          ...
          <div className="A-sv-card">
            <div className="lbl" style={{ fontFamily:'Geist Mono, monospace',fontSize:10.5,color:'var(--a-ink-400)',letterSpacing:'0.08em',textTransform:'uppercase' }}>META TOTAL</div>
            <div className="amt" style={{ marginTop: 10 }}>42%</div>
            <div className="bar" style={{ marginTop: 14 }}><span style={{ width:'42%', background:'linear-gradient(90deg, var(--accent), var(--accent-2))' }}/></div>
            <div className="foot"><span>R$ 40.300</span><span>R$ 112.000</span></div>
          </div>
          <div className="A-sv-card">
            <div className="lbl" style={{ fontFamily:'Geist Mono, monospace',fontSize:10.5,color:'var(--a-ink-400)',letterSpacing:'0.08em',textTransform:'uppercase' }}>RENDIMENTO (MÊS)</div>
            <div className="amt" style={{ marginTop: 10, color: '#10b981' }}>+ R$ 412</div>
            <div className="foot" style={{ marginTop: 14 }}><span>+1.02%</span><span>vs. mês anterior</span></div>
          </div>
        </div>
              ...
              <div key={c.name} className="A-sv-card">
                <div className="h">
                  <div style={{ display:'flex',alignItems:'center',gap:12 }}>
                    <div className="emoji" style={{ background: `color-mix(in oklab, ${c.color} 15%, transparent)` }}>{c.emoji}</div>
                    <div><div className="name">{c.name}</div><div className="target">Meta: {c.eta}</div></div>
                  </div>
                  <button className="A-chip" style={{ height: 30 }}>+ Aportar</button>
                </div>
                <div><span className="amt">{fmt(c.cur)}</span><span className="of">/ {fmt(c.tgt)}</span></div>
                <div className="bar"><span style={{ width: `${pct}%`, background: c.color }}/></div>
                <div className="foot"><span>{pct}% DO OBJETIVO</span><span>FALTAM {fmt(c.tgt-c.cur)}</span></div>
              </div>
            );
          })}
        </div>
...
function ProfileA({ tweaks }) {
  return (
    <Shell active="profile" tweaks={tweaks}>
      ...
      <main className="A-main">
        <div className="A-top">
          <div><h1>Meu Perfil</h1><div className="sub">Gerencie suas informações pessoais e preferências</div></div>
        ...
        <div className="A-prof-grid">
          <div className="A-card" style={{ height:'fit-content' }}>
            <div className="A-prof-avatar">DC<div className="cam">📷</div></div>
            <div style={{ textAlign:'center',marginTop:16 }}>
              <div style={{ fontSize:18,fontWeight:600,letterSpacing:'-0.02em',color:'var(--a-ink-900)' }}>davyd camargo</div>
              <div style={{ fontSize:13,color:'var(--a-ink-500)',marginTop:4 }}>davydfontoura@gmail.com</div>
              <div style={{ marginTop:16,display:'inline-flex',padding:'4px 12px',borderRadius:999,background:'color-mix(in oklab, #10b981 12%, transparent)',color:'#059669',fontSize:11,fontFamily:'Geist Mono, monospace',letterSpacing:'0.06em',textTransform:'uppercase',fontWeight:500 }}>● Ativa e verificada</div>
            </div>
          </div>
          <div style={{ display:'flex',flexDirection:'column',gap:16 }}>
            <div className="A-card">
              <div className="A-section-h"><span className="i">◉</span><h3>Informações Pessoais</h3><span className="meta">ID: 7c102b5d…</span></div>
              <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14 }}>
                <div className="A-field"><label>Nome completo</label><input defaultValue="davyd camargo"/></div>
                <div className="A-field"><label>Username</label><input defaultValue="@davydfontoura"/></div>
                <div className="A-field" style={{ gridColumn:'span 2' }}><label>E-mail (não editável)</label><input defaultValue="davydfontoura@gmail.com" disabled/></div>
              </div>
            </div>
            <div className="A-card">
              <div className="A-section-h"><span className="i">✓</span><h3>Segurança</h3></div>
              <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14 }}>
                <div className="A-field"><label>Senha atual</label><input type="password" defaultValue="••••••••••"/></div>
                <div className="A-field"><label>Nova senha</label><input type="password" placeholder="Digite a nova senha"/></div>
              </div>
              <div style={{ marginTop:14,padding:14,borderRadius:10,background:'var(--a-bg)',display:'flex',alignItems:'center',justifyContent:'space-between' }}>
                <div><div style={{ fontSize:13,fontWeight:500,color:'var(--a-ink-900)' }}>Autenticação em 2 fatores</div><div style={{ fontSize:12,color:'var(--a-ink-500)',marginTop:2 }}>Adicione uma camada extra de proteção</div></div>
                <button className="A-chip">Ativar</button>
              </div>
            </div>
            <div className="A-card A-danger">
              <div className="A-section-h"><span className="i">⚠</span><h3>Zona de Perigo</h3></div>
              <p>Ao excluir sua conta, todos os seus dados (transações, categorias, caixinhas e perfil) serão removidos permanentemente. Esta ação não pode ser desfeita.</p>
...
function AuthA({ tweaks }) {
  ...
  return (
    <div className="A-auth" data-theme={t.theme} style={{ '--accent': a.accent, '--accent-2': a.accent2 }}>
      <style>{AUTH_STYLE}</style>
      <div className="A-auth-form">
        <div className="A-auth-brand"><img src="public/logo-expense-tracker.png" alt=""/><b>Expense Tracker</b></div>
        <h1>{mode==='signup' ? 'Criar conta' : 'Entrar'}</h1>
        <div className="sub">{mode==='signup' ? 'Comece a organizar suas finanças em menos de 5 minutos.' : 'Bem-vindo de volta. Continue de onde parou.'}</div>

        <div className="A-auth-tabs">
          <button className={mode==='signup'?'active':''} onClick={()=>setMode('signup')}>Cadastrar</button>
        ...
        {mode==='signup' && <div className="A-field"><label>Nome completo</label><input placeholder="Como devemos te chamar?"/></div>}
        <div className="A-field"><label>E-mail</label><input type="email" placeholder="Digite seu email"/></div>
        <div className="A-field"><label>Senha</label><input type="password" placeholder="Mínimo 8 caracteres"/></div>

        {mode==='login' && (
          <div className="row">
            <label style={{ display:'inline-flex',gap:8,alignItems:'center',color:'var(--a-ink-700)' }}>
              <input type="checkbox" defaultChecked/> Lembrar de mim
            </label>
            <a style={{ color:'var(--accent)',fontWeight:500,cursor:'pointer' }}>Esqueci minha senha</a>
          </div>
        )}

        <button className="A-auth-submit">{mode==='signup' ? 'Criar conta grátis' : 'Entrar'} →</button>

        <div className="A-auth-foot">
          {mode==='signup' ? 'Já tem conta? ' : 'Novo por aqui? '}
          <a onClick={()=>setMode(mode==='signup'?'login':'signup')}>{mode==='signup' ? 'Entre' : 'Crie uma conta'}</a>
        </div>
      </div>

      <div className="A-auth-side">
        <div>
          <div style={{ display:'inline-flex',alignItems:'center',gap:8,padding:'6px 12px',borderRadius:999,background:'rgba(255,255,255,0.08)',border:'1px solid rgba(255,255,255,0.12)',fontFamily:'Geist Mono, monospace',fontSize:11,letterSpacing:'0.06em',color:'rgba(255,255,255,0.75)',marginBottom:32 }}>
            <span style={{ width:6,height:6,borderRadius:'50%',background:'#10b981',boxShadow:'0 0 0 3px rgba(16,185,129,0.2)' }}/> 100% OPEN SOURCE
          </div>
          <h2>Seu dinheiro, <em>sob controle.</em></h2>
          <p>Dashboard inteligente, importação de extratos em CSV, categorias customizáveis e segurança Supabase. Sem cartão de crédito. Sem pegadinha.</p>
        </div>
        <div className="A-auth-stats">
          <div><div className="s-v">5min</div><div className="s-l">Até o primeiro relatório</div></div>
          <div><div className="s-v">R$ 0</div><div className="s-l">Grátis para sempre</div></div>
          <div><div className="s-v">∞</div><div className="s-l">Categorias customizáveis</div></div>
          <div><div className="s-v">RLS</div><div className="s-l">Segurança Supabase</div></div>
        </div>
      </div>
    </div>
  );
...
Object.assign(window, { DashboardA, TransactionsA, CategoriesA, SavingsA, ProfileA, AuthA, KPIS, CATEGORIES, MONTHLY, TXS, fmt, Donut });
